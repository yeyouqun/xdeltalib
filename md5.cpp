 ...
 */
	...
	#define	FETCH_32(p)	(*(const unsigned int *)(p))
#else
	#define	FETCH_32(p)						\
		(((unsigned int)*((const unsigned char *)(p))) |		\
...
static void MD5Transform(unsigned int, unsigned int, unsigned int, unsigned int,
    const unsigned char [64], MD5_CTX *);
 ...
 */
static void
MD5Transform(unsigned int a, unsigned int b, unsigned int c, unsigned int d,
    const unsigned char block[64], MD5_CTX *context)
{
	/* Register (instead of array) is a win in most cases */
	register unsigned int x0, x1, x2, x3, x4, x5, x6, x7;
	register unsigned int x8, x9, x10, x11, x12, x13, x14, x15;
