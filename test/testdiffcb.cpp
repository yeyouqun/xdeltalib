...
void diff_callback(char * data, unsigned datalen, unsigned long long offset, void * priv)
{
	file_writer * p_cstor_writer = (file_writer *)priv;
	p_cstor_writer->seek_file(offset, FILE_BEGIN);
	p_cstor_writer->write_file((xdelta::uchar_t*)data, datalen);
}
///////////////////////////////////////////////////////////////
void test_single_round(const std::string & srcfile, const std::string & tgtfile)
{
	...
	inner_data = xdelta_start_xdelta(hash_result, blklen
							, diff_callback, (void *)p_cstor_writer);
	xdelta_free_hashes(hash_result);
	...
	for (xit_t * p = xdelta_result; p != 0; p = p->next) {
		if (p->type == DT_IDENT) {
			...
			// 在加调函数中已经处理了。回调函数处理的结果与这里的处理是匹配的。
			SYNC_DIFF(p);
		}
	}
